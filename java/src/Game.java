...
public class Game {
    ...
    private char lastPlayedSymbol = Players.NO_PLAYER_SYMBOL;
    ...
    private void validatePlayerNotOverrideAlreadyPlayedTitle(int x, int y) throws InvalidPosition {
        if (board.TileAt(x, y).Symbol != Players.NO_PLAYER_SYMBOL) {
    ...
    private boolean isFirstMove() {
        return lastPlayedSymbol == Players.NO_PLAYER_SYMBOL;
    }

    public char Winner() {
        //if the positions in first row are taken
        if (board.TileAt(0, 0).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(0, 1).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(0, 2).Symbol != Players.NO_PLAYER_SYMBOL) {
        ...
        //if the positions in first row are taken
        if (board.TileAt(1, 0).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(1, 1).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(1, 2).Symbol != Players.NO_PLAYER_SYMBOL) {
        ...
        //if the positions in first row are taken
        if (board.TileAt(2, 0).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(2, 1).Symbol != Players.NO_PLAYER_SYMBOL && board.TileAt(2, 2).Symbol != Players.NO_PLAYER_SYMBOL) {
            //if middle row is full with same symbol
        ...
        return Players.NO_PLAYER_SYMBOL;
    }
}
